...
func (s *server) getVFS(what string, sshConn *ssh.ServerConn) (VFS *vfs.VFS) {
	...
	if sshConn.Permissions == nil || sshConn.Permissions.Extensions == nil {
		fs.InfofCtx(s.ctx, what, "SSH Permissions Extensions not found")
		return nil
	}
	key := sshConn.Permissions.Extensions["_vfsKey"]
	if key == "" {
		fs.InfofCtx(s.ctx, what, "VFS key not found")
		return nil
	}
	VFS = s.proxy.Get(key)
	if VFS == nil {
		fs.InfofCtx(s.ctx, what, "failed to read VFS from cache")
		return nil
	}
	return VFS
}
...
func (s *server) acceptConnection(nConn net.Conn) {
	...
	if err != nil {
		fs.ErrorfCtx(s.ctx, what, "SSH login failed: %v", err)
		return
	}

	fs.InfofCtx(s.ctx, what, "SSH login from %s using %s", sshConn.User(), sshConn.ClientVersion())
	...
	if c.vfs == nil {
		fs.InfofCtx(s.ctx, what, "Closing unauthenticated connection (couldn't find VFS)")
		_ = nConn.Close()
		return
	}
...
func (s *server) acceptConnections() {
	for {
		...
		if err != nil {
			...
			fs.ErrorfCtx(s.ctx, nil, "Failed to accept incoming connection: %v", err)
			continue
		}
...
func (s *server) configure() (err error) {
	...
	if s.opt.AuthorizedKeys != "" && proxy.Opt.AuthProxy == "" {
		...
		fs.LogfCtx(s.ctx, nil, "Loaded %d authorized keys from %q", len(authorizedKeysMap), authKeysFile)
	}
	...
	s.config = &ssh.ServerConfig{
		...
		PasswordCallback: func(c ssh.ConnMetadata, pass []byte) (*ssh.Permissions, error) {
			fs.DebugfCtx(s.ctx, describeConn(c), "Password login attempt for %s", c.User())
			if s.proxy != nil {
		...
		PublicKeyCallback: func(c ssh.ConnMetadata, pubKey ssh.PublicKey) (*ssh.Permissions, error) {
			fs.DebugfCtx(s.ctx, describeConn(c), "Public key login attempt for %s", c.User())
			if s.proxy != nil {
				//query the proxy for the config
		...
		AuthLogCallback: func(conn ssh.ConnMetadata, method string, err error) {
			...
			fs.DebugfCtx(s.ctx, describeConn(conn), "ssh auth %q from %q: %s", method, conn.ClientVersion(), status)
		},
	...
	for _, keyPath := range keyPaths {
		...
		if err != nil && len(s.opt.HostKeys) == 0 {
			fs.DebugfCtx(s.ctx, nil, "Failed to load %q: %v", keyPath, err)
		...
		fs.DebugfCtx(s.ctx, nil, "Loaded private key from %q", keyPath)
...
func (s *server) Serve() (err error) {
	fs.LogfCtx(s.ctx, nil, "SFTP server listening on %v\n", s.listener.Addr())
	s.acceptConnections()
	close(s.stopped)
	return nil
}
