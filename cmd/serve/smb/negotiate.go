...
// capLargeMTU is SMB2_GLOBAL_CAP_LARGE_MTU ([MS-SMB2] 2.2.4). Without it a client
// caps every READ/WRITE at 64 KiB regardless of the advertised MaxReadSize; it is
// an SMB 2.1+ capability, so it is not advertised for the 2.0.2 dialect.
...
func (c *conn) handleNegotiate(h header, body []byte) (uint32, []byte) {
	...
	return statusSuccess, negotiateRespBody(dialect, c.server.serverGUID, c.server.opt.User != "")
}

// negotiateCapabilities returns the SMB2 capabilities we advertise for a
// negotiated dialect. LARGE_MTU (multi-credit IO above 64 KiB) is an SMB 2.1+
// capability. The IOCTL VALIDATE_NEGOTIATE_INFO response must report the same
// value, so both sites use this.
func negotiateCapabilities(dialect uint16) uint32 {
	switch dialect {
	case dialect210, dialect300, dialect302:
		return capLargeMTU
	}
	return 0
}

// negotiateSecurityMode returns the SecurityMode we advertise. Signing is
// required when a user is configured (there are then no guest sessions, which
// couldn't sign); guest servers advertise signing as enabled but not required.
// The IOCTL VALIDATE_NEGOTIATE_INFO response must report the same value.
func negotiateSecurityMode(signingRequired bool) uint16 {
	if signingRequired {
		return negotiateSigningEnabled | negotiateSigningRequired
	}
	return negotiateSigningEnabled
}

// negotiateRespBody builds the NEGOTIATE response body for the given dialect.
// When signingRequired is set (a user is configured, so there are no guest
// sessions) it advertises SMB2_NEGOTIATE_SIGNING_REQUIRED, telling clients to
// sign every request; guest servers only advertise signing as enabled, since a
// guest session has no key to sign with.
func negotiateRespBody(dialect uint16, guid [16]byte, signingRequired bool) []byte {
	resp := make([]byte, 64)                                        // fixed portion; no security buffer payload
	le.PutUint16(resp[0:2], 65)                                     // StructureSize (fixed magic value)
	le.PutUint16(resp[2:4], negotiateSecurityMode(signingRequired)) // SecurityMode
	le.PutUint16(resp[4:6], dialect)                                // DialectRevision
	le.PutUint16(resp[6:8], 0)                                      // NegotiateContextCount
	copy(resp[8:24], guid[:])                                       // ServerGuid
	le.PutUint32(resp[24:28], negotiateCapabilities(dialect))       // Capabilities
...
func (c *conn) smb1NegotiateResponse() []byte {
	body := negotiateRespBody(dialectWildcard, c.server.serverGUID, c.server.opt.User != "")
