...
func (c *conn) handleQueryInfo(h header, body []byte) (uint32, []byte) {
	...
	infoType := body[2]
	infoClass := body[3]
	outputBufferLength := int(le.Uint32(body[4:8]))
	...
	var info []byte
	fixed := 0 // length of the fixed part, for the classes that end in a variable-length name
	switch infoType {
	case infoTypeFile:
		attrs, size, mtime := nodeAttrs(of.node)
		switch infoClass {
		case classFileAll:
			info, fixed = fileAllInfo(attrs, size, mtime, of.node), 100
	...
	case infoTypeFilesystem:
		...
		case classFsAttribute:
			info, fixed = fsAttributeInfo(), 12
		case classFsVolume:
			info, fixed = fsVolumeInfo(c.server.serverGUID), 18
		case classFsDevice:
			info = fsDeviceInfo()
		default:
			return statusNotSupported, errorResponseBody()
	...
	// The reply must fit the client's buffer. Windows sizes it for just the fixed
	// part when that is all it wants (GetFileInformationByHandle reads the volume
	// serial number this way) and rejects a longer reply as an invalid network
	// response, so cut a trailing name short and say so.
	status := statusSuccess
	if len(info) > outputBufferLength {
		if fixed == 0 || outputBufferLength < fixed {
			return statusInfoLengthMismatch, errorResponseBody()
		}
		info, status = info[:outputBufferLength], statusBufferOverflow
	}
	return status, infoResponseBody(info)
}
