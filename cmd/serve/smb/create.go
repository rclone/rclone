...
type openFile struct {
	...
	handle vfs.Handle // open IO handle for files (nil for directories); safe for concurrent IO
	isDir  bool

	// mu guards the mutable state below: the path changes on rename, deleteOnClose
	// on a disposition SET_INFO, and the enumeration progress during QUERY_DIRECTORY
	// -- so a handle can be touched by one request while the connection serves
	// others concurrently. It is not held during the (immutable) IO on handle.
	mu            sync.Mutex
	path          string // VFS path (forward-slash, no leading slash)
	deleteOnClose bool
	dirEntries    []vfs.Node // directory enumeration state for QUERY_DIRECTORY
	dirPos        int
	dirLoaded     bool
}
...
func (c *conn) handleCreate(h header, body []byte) (uint32, []byte) {
	...
	case !exists && createOptions&optDirectoryFile != 0:
		newNode, created, err := c.server.mkdir(path)
		if err != nil {
			return mapVFSError(err), errorResponseBody()
		}
		if !created && (createDisposition == dispCreate || !newNode.IsDir()) {
			return statusObjectNameCollision, errorResponseBody()
		}
	...
	if createOptions&optDeleteOnClose != 0 {
		if c.dirHasEntries(of) {
			return statusDirectoryNotEmpty, errorResponseBody()
		}
...
// mkdir makes the directory at path unless it exists already, returning its node
// and whether this call created it.
//
// Directories are made one at a time because the VFS looks for the directory and
// then adds it without holding a lock in between. Clients creating the same
// directory at once - a multi-transfer copy into a new directory does - would
// each add a node for it, the last displacing the others along with any files
// already created in them that have not reached the backend yet.
func (s *Server) mkdir(path string) (node vfs.Node, created bool, err error) {
	s.mkdirMu.Lock()
	defer s.mkdirMu.Unlock()
	if node, err = s.vfs.Stat(path); err == nil {
		return node, false, nil
	}
	if err = s.vfs.Mkdir(path, 0777); err != nil {
		return nil, false, err
	}
	node, err = s.vfs.Stat(path)
	return node, true, err
}

// dirHasEntries reports whether of is a directory that still has entries.
//
// Such a directory can't be deleted, and a client asking to must be told when it
// asks: the delete happens at CLOSE, and Windows ignores the status of a CLOSE.
func (c *conn) dirHasEntries(of *openFile) bool {
	if !of.isDir {
		return false
	}
	nodes, err := c.listDir(of.path)
...
// handleCount returns the number of open handles on the connection. The cap
// check in handleCreate may still be raced past by concurrent creates, but only
// by at most the in-flight request count, which is bounded.
