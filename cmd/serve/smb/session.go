...
func (c *conn) handleSessionSetup(h header, body []byte) (status uint32, sessionID uint64, respBody []byte) {
	...
	// A connection serves requests concurrently, so the session table and each
	// session's fields are accessed under the lock. Copy out the challenge (set on
	// the first round trip) for the AUTHENTICATE validation below.
	c.mu.Lock()
	...
	if ntlmMessageType(token) == ntlmTypeAuthenticate {
		sessionFlags, signKey, ok := c.authenticate(token, challenge)
		if !ok {
			return statusLogonFailure, sessionID, errorResponseBody()
		}
		if c.server.opt.User != "" {
			// Mark the session authenticated and remember its signing key, so later
			// commands pass the auth + signing gate in handleCommand.
			c.mu.Lock()
			sess.authed = true
			sess.signKey = signKey
			c.mu.Unlock()
		}
	...
	// First round trip: generate and remember this session's challenge, then send
	// it back in the same wrapping the client used.
	c.mu.Lock()
	_, _ = rand.Read(sess.challenge[:])
	raw := sess.challenge
	c.mu.Unlock()
	ntlmChallenge := buildNTLMChallenge(raw)
	reply := ntlmChallenge
	if !rawNTLM {
		reply = buildNegTokenResp(negStateAcceptIncomplete, true, ntlmChallenge)
	}
	return statusMoreProcessingRequired, sessionID, sessionSetupRespBody(0, reply)
}

// authenticate validates an NTLMSSP AUTHENTICATE token against the session's
// challenge. It returns the session flags to report, the derived SMB signing
// key (nil for guest), and whether authentication succeeded. With no --user
// configured every client is accepted as a guest.
func (c *conn) authenticate(token []byte, challenge [8]byte) (sessionFlags uint16, signKey []byte, ok bool) {
	if c.server.opt.User == "" {
		return sessionFlagIsGuest, nil, true
	}
	...
	if !validateNTLMv2(responseKeyNT, challenge, auth.ntResponse) {
		return 0, nil, false
	}
	// Derive the SMB signing key for this session so its messages can be signed.
	sessionKey := exportedSessionKey(responseKeyNT, auth.ntResponse[:16], auth.flags, auth.encSessionKey)
	switch c.getDialect() {
	case dialect300, dialect302:
		signKey = kdf(sessionKey, []byte("SMB2AESCMAC\x00"), []byte("SmbSign\x00"))
	default:
		signKey = sessionKey
	}
	return 0, signKey, true
}
