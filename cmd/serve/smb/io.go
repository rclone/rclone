...
func (c *conn) handleClose(h header, body []byte) (uint32, []byte) {
	...
	if of != nil {
		...
		// macOS asks for the attributes on every close, and logs a kernel message for
		// each reply without them. They are read once the handle is closed, so the
		// size is final.
		if le.Uint16(body[2:4])&closeFlagPostQueryAttrib != 0 {
			resp = closeResponseBody(of.node)
		}
