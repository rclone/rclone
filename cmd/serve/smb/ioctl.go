...
func (c *conn) handleIoctl(h header, body []byte) (uint32, []byte) {
	...
	// VALIDATE_NEGOTIATE_INFO response ([MS-SMB2] 2.2.32.1): the values MUST match
	// what we sent in the NEGOTIATE response, or the client treats it as tampering
	// and disconnects.
	dialect := c.getDialect()
	out := make([]byte, 24)
	le.PutUint32(out[0:4], negotiateCapabilities(dialect))                   // Capabilities
	copy(out[4:20], c.server.serverGUID[:])                                  // ServerGuid
	le.PutUint16(out[20:22], negotiateSecurityMode(c.server.opt.User != "")) // SecurityMode
	le.PutUint16(out[22:24], dialect)                                        // Dialect
