...
func (s *HTTP) serveDir(w http.ResponseWriter, r *http.Request, dirRemote string) {
	...
	// Set the Last-Modified header to the timestamp, unless it is the
	// configured fallback for an unknown modtime, which would otherwise
	// make every listing of this directory report the same static date
	dirModTime := dir.ModTime()
	if dirModTime.Equal(time.Time(fs.GetConfig(ctx).DefaultTime)) {
		dirModTime = time.Now()
	}
