...
S3 server supports Signature Version 4 authentication. Just
 use ` + `--s3-auth accessKey1-secretKey1,accessKey2-secretKey2` + ` and
...
folders under root, but you can't create new folders under other folders.

When using PutObject or DeleteObject, rclone will automatically create 
and clean up empty folders by the prefix. If you don't want to clean up 
