...
func TestRun(t *testing.T) {
	...
	t.Run("Call w/Password", func(t *testing.T) {
		...
		// Regression: the underlying fs.Fs must also be a fresh
		// instance from fs/cache, not the one created for the
		// original auth. Before this fix, fsString didn't include
		// the auth hash, so fs/cache reused the stale backend (with
		// stale config) even after the auth changed.
		if vfs.Fs() == vfs2.Fs() {
			t.Error("fs/cache returned the stale backend after auth change")
		}

		// Defense-in-depth: if a cached entry's pwHash somehow doesn't
		// match the supplied auth (eg a hash collision on the cache
		// key), Call must reject it. Simulate by corrupting the cached
		// pwHash.
