...
func (r *results) checkBase32768() {
	...
	if err != nil {
		fs.LogfCtx(ctx, nil, "Failed to make temp dir: %v", err)
		return
	}
	defer func() {
		_ = os.RemoveAll(dir)
	...
	for _, c := range safeAlphabet {
		...
		if err != nil {
			fs.LogfCtx(ctx, nil, "write %q failed: %v", fileName, err)
			return
		}
		n++
	}
	...
	if err != nil {
		fs.LogfCtx(ctx, nil, "Failed to make local fs: %v", err)
		return
	}
	...
	if err != nil {
		fs.LogfCtx(ctx, nil, "Failed to make remote fs: %v", err)
		return
	}
	...
	defer func() {
		...
		if err != nil {
			fs.LogfCtx(ctx, nil, "Failed to purge test directory: %v", err)
			return
		}
	}()
	...
	if err != nil {
		fs.LogfCtx(ctx, nil, "Failed to sync remote fs: %v", err)
		return
	}
	...
	if err != nil {
		fs.LogfCtx(ctx, nil, "Failed to check remote fs: %v", err)
		return
	}
