...
**NB** this can create undeletable files and other hazards - use with care!`,
	...
	Run: func(command *cobra.Command, args []string) {
		...
		for i := range args {
			...
			fs.InfofCtx(context.Background(), f, "Created temporary directory for test files: %s", tempDirPath)
...
func (r *results) WriteJSON() {
	...
	if f, err := os.Create(writeJSON); err != nil {
		fs.ErrorfCtx(context.Background(), r.f, "Creating JSON file failed: %s", err)
	} else {
		...
		if err != nil {
			fs.ErrorfCtx(context.Background(), r.f, "Writing JSON file failed: %s", err)
		}
	}
	fs.InfofCtx(context.Background(), r.f, "Wrote JSON file: %s", writeJSON)
}
...
func (r *results) checkStringPositions(k, s string) {
	fs.InfofCtx(context.Background(), r.f, "Writing position file 0x%0X", s)
	...
	for _, pos := range internal.PositionList {
		...
		if writeError != nil {
			...
			fs.InfofCtx(context.Background(), r.f, "Writing %s position file 0x%0X Error: %s", pos.String(), s, writeError)
		} else {
			fs.InfofCtx(context.Background(), r.f, "Writing %s position file 0x%0X OK", pos.String(), s)
		}
		obj, getErr := r.f.NewObject(r.ctx, path)
		if getErr != nil {
			res.GetError[pos] = getErr.Error()
			fs.InfofCtx(context.Background(), r.f, "Getting %s position file 0x%0X Error: %s", pos.String(), s, getErr)
		} else {
			if obj.Size() != 50 {
				res.GetError[pos] = fmt.Sprintf("invalid size %d", obj.Size())
				fs.InfofCtx(context.Background(), r.f, "Getting %s position file 0x%0X Invalid Size: %d", pos.String(), s, obj.Size())
			} else {
				fs.InfofCtx(context.Background(), r.f, "Getting %s position file 0x%0X OK", pos.String(), s)
			}
		}
...
func (r *results) checkControls() {
	fs.InfofCtx(context.Background(), r.f, "Trying to create control character file names")
	...
	wg.Wait()
	r.checkControlsList()
	fs.InfofCtx(context.Background(), r.f, "Done trying to create control character file names")
}

func (r *results) checkControlsList() {
	l, err := r.f.List(context.TODO(), "")
	if err != nil {
		fs.ErrorfCtx(context.Background(), r.f, "Listing control character file names failed: %s", err)
		return
	}
	...
	for path := range namesMap {
		...
		if g := positionLeftRe.FindStringSubmatch(path); g != nil {
			...
			fs.InfofCtx(context.Background(), r.f, "Unknown path %q", path)
			continue
		}
		var hexValue []byte
		for ; len(hex) >= 2; hex = hex[2:] {
			if b, err := strconv.ParseUint(hex[:2], 16, 8); err != nil {
				fs.InfofCtx(context.Background(), r.f, "Invalid path %q: %s", path, err)
				continue
			} else {
		...
		if hex != "" {
			fs.InfofCtx(context.Background(), r.f, "Invalid path %q", path)
			continue
		}
	...
	if len(namesMap) > 0 {
		fs.InfofCtx(context.Background(), r.f, "Found additional control character file names:")
		for name := range namesMap {
			fs.InfofCtx(context.Background(), r.f, "%q", name)
		}
	}
}
	...
	i := sort.Search(len(name), func(i int) (fail bool) {
		defer func() {
			if err := recover(); err != nil {
				fs.InfofCtx(context.Background(), r.f, "Couldn't write file with name length %d: %v", i, err)
				fail = true
			}
		}()
		...
		fs.InfofCtx(context.Background(), r.f, "Wrote file with name length %d", i)
		err = o.Remove(context.Background())
		if err != nil {
			fs.ErrorfCtx(context.Background(), o, "Failed to remove test file")
		}
		return false
	})
	r.maxFileLength[characterLength-1] = i - 1
	fs.InfofCtx(context.Background(), r.f, "Max file length is %d when writing %d byte characters %q", r.maxFileLength[characterLength-1], characterLength, character)
}

func (r *results) checkStreaming() {
	putter := r.f.Put
	if r.f.Features().PutStream != nil {
		fs.InfofCtx(context.Background(), r.f, "Given remote has specialized streaming function. Using that to test streaming.")
	...
	if err != nil {
		fs.InfofCtx(context.Background(), r.f, "Streamed file failed to upload (%v)", err)
		r.canStream = false
		return
	}
	...
	for _, Hash := range types {
		...
		if err != nil {
			fs.InfofCtx(context.Background(), r.f, "Streamed file failed when getting hash %v (%v)", Hash, err)
			r.canStream = false
			return
		}
		if !hash.Equals(hashes[Hash], sum) {
			fs.InfofCtx(context.Background(), r.f, "Streamed file has incorrect hash %v: expecting %q got %q", Hash, hashes[Hash], sum)
			r.canStream = false
			return
		}
	}
	if int64(len(contents)) != objR.Size() {
		fs.InfofCtx(context.Background(), r.f, "Streamed file has incorrect file size: expecting %d got %d", len(contents), objR.Size())
		r.canStream = false
		return
	}
	r.canStream = true
