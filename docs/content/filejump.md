...
FileJump doesn't support setting custom modification times via the API. The modification time will be set to the upload time by the server. This means that `--update` and similar flags that rely on modification times may not work as expected.
...
- **Move**: ✅ Supported via copy + delete (server-side move API is unreliable)
- **Directory Move**: ❌ Not supported - rclone will fall back to recursive copy/delete
...
- FileJump doesn't provide file hashes
