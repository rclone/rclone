...
## Working with classification labels

Google Drive supports [classification labels](https://support.google.com/a/answer/9292382)
which can be used to categorise files, enforce data loss prevention (DLP) policies,
and set retention rules.

### Discovering label and field IDs

Before configuring `--drive-metadata-labels` or `--drive-static-label` you need the
label ID, field ID, and — for selection fields — the selection option ID. The easiest
way to discover them is to use rclone itself:

1. Apply the label manually to any file in Google Drive (via the Drive UI or Google Admin).
2. Read it back with rclone:

```console
rclone lsjson --metadata --drive-metadata-labels=read remote:path/to/file
```
...
[{
  "Path": "file.md",
  "Metadata": {
    "labels": "[{\"fields\":{\"FIELD_ID\":{\"id\":\"FIELD_ID\",\"selection\":[\"SELECTION_OPTION_ID\"],\"valueType\":\"selection\"}},\"id\":\"LABEL_ID\",\"revisionId\":\"1\"}]"
  }
}]
```

The `labels` metadata value is itself a JSON array. Each element represents one label
applied to the file. The fields you need are:

- **Label ID**: the top-level `id` field of each label object
- **Field ID**: the key inside `fields` (e.g. `FIELD_ID`)
- **Selection option ID**: the value inside `selection` (e.g. `SELECTION_OPTION_ID`)

The `revisionId`, `valueType`, and the field-level `id` inside each field object are
read-only metadata returned by the API and are not needed when writing labels.

### Applying a fixed label to all uploads

Use `--drive-static-label` to stamp a label on every file rclone writes, regardless
of the source backend. This is useful when all files synced to Drive must carry a
sensitivity or classification label for compliance or DLP reasons, but the source
(e.g. a local filesystem or S3) has no label metadata to copy from.

The value is a single JSON label object (not an array) containing only the fields
rclone needs to write the label:

```json
{"id":"LABEL_ID","fields":{"FIELD_ID":{"selection":["SELECTION_OPTION_ID"]}}}
```

Add it to your remote config:

```ini
[gdrive]
type = drive
static_label = {"id":"LABEL_ID","fields":{"FIELD_ID":{"selection":["SELECTION_OPTION_ID"]}}}
```

Or pass it as a flag:

```console
rclone sync /local/path gdrive: --drive-static-label '{"id":"LABEL_ID","fields":{"FIELD_ID":{"selection":["SELECTION_OPTION_ID"]}}}'
```

The label must already exist in the Drive label schema — rclone will not create it.
The `--drive-metadata-labels` flag does not need to be enabled for `--drive-static-label` to work.
