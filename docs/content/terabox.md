...
This is a Backend for Terabox Cloud storage. 

**Notice** This integrations is not official, because official integration required special `client id` and `client secret` which only can be provided by Terabox. I made a few requests for this, but did not got a responce. If Terabox will provide the keys for official integration, will update the integration.

Paths are specified as `remote:path`

Paths may be as deep as required, e.g. `remote:directory/subdirectory`.

If you paid for subscription, but application still show you are limited for 4GB upload, wait 24 hours or remove from config file lines: `premium` and `premium_expire`.

## Limitations

Max upload filesize is 4GB for free accounts and 128GB for premium accounts.

Max name length: 255 symbols.

Download in multithread mode and single thread mode has the same speed result. I got about 1.7 - 1.9 MB/s for standart multithread download and the same result for one thread (with key`--multi-thread-streams 1`). About 8MB/s for premium account.

Upload speed about 15MB/s for both (free and premium).

Cookie live period is 1 year.

Minimum filesize is 1 byte (0 byte files not supported).

For some regions you need to change backend domain to `https://nephobox.com` (if you get multiple errors for upload/download, try to use alternative domain).

If you faced with upload issues when upload is stucking, try to use `--timeout=15s` option.

Server side Copy works in async mode, that's mean if try run concurrency this method, you will face with error `couldn't copy file: You currently have unfinished tasks, please operate after completion`. Method overwrite existing files with source. Server side Move do not overwrite existing files and works in sync mode.
...
Terabox backend domain.

Properties:

- Config:      domain
- Env Var:     RCLONE_TERABOX_DOMAIN
- Type:        string
...
Set extra verbose level from 0 to 1.
* 0 - none
...
- Type:        Encoding
- Default:     Slash, LtGt, DoubleQuote, Colon, Question, Asterisk, Pipe, BackSlash, CrLf, Del, Ctl, LeftCrLfHtVt, RightCrLfHtVt, RightSpace, RightPeriod, InvalidUtf8, Dot
