...
### Healing writes {#heal-writes}

With `create_policy = all`, a new file is written to every writable
upstream. If one of those writes fails (for example the upstream was
briefly unavailable), the file ends up existing on some upstreams but
not others. By default, updating that file afterwards only writes to
the upstreams selected by `action_policy` which already have it, so the
missing upstream is never repaired.

Setting the `heal_writes` option makes updates also create the file on
any upstream selected by `create_policy` that is still missing it,
repairing the union the next time the file is written.
