...
By default, Rclone follows symbolic links and copies the content they point
to as regular files or directories. Using the --sftp-links (or --links) flag,
Rclone instead creates real symlinks on the SFTP server,
preserving the exact target paths (relative or absolute).

#### How it works

When `--sftp-links` is enabled:

1. **Local/Cloud → SFTP**: Source symlinks are translated to physical
   symlinks on the SFTP server using the SFTP protocol's symlink creation
   command. The symlink target path is preserved as-is (whether relative
   or absolute).

2. **SFTP → SFTP**: Physical symlinks are read from the source server and
   recreated as physical symlinks on the destination server, preserving
   the original target paths. (But it can depend on server implementation,
   see Important notes below.)

3. **SFTP → Local/Cloud**: Physical symlinks from the SFTP server are
   translated to `.rclonelink` files (text files containing the symlink
   target path) when copying to backends that don't support native symlinks.

#### Example usage

Copy a directory with symlinks from local filesystem to SFTP server:

```sh
rclone sync --links /local/path remote:/destination
```

Copy a directory with symlinks from SFTP server to local filesystem:

```sh
rclone sync --links remote:/path /local/destination
```

Synchronize symlinks between two SFTP servers:

```sh
rclone sync --links remote1:/path remote2:/path
```

Copy symlinks from SFTP to a cloud backend (creates `.rclonelink` files):

```sh
rclone sync --links remote:/path s3:bucket/path
```

Copy symlinks from a cloud backend (with `.rclonelink` files) to SFTP (creates real symlinks):

```sh
rclone sync --links s3:bucket/path remote:/path
```

#### Important notes

- **Target path preservation**: Symlink target paths are copied verbatim.
  Relative symlinks remain relative, and absolute symlinks remain absolute.

- **Directory structure dependencies**: Relative symlinks may break if the
  destination directory structure differs from the source.

- **Absolute symlinks**: When copying between different servers, absolute
  symlink paths may not be valid on the destination server if the
  directory structure differs.

- **Broken symlinks**: Symlinks pointing to non-existent targets are
  copied anyway. The symlink itself is preserved even if its target
  doesn't exist.

- **Server compatibility**: Not all SFTP servers support symlink creation.
  The server must support the SSH_FXP_SYMLINK operation. Most modern SFTP
  servers (OpenSSH, SFTPGo, ProFTPD with mod_sftp) support this.

- **Path normalization**: Some SFTP servers may normalize or modify symlink
  target paths when storing them. For example, some servers convert relative
  paths to absolute paths within their chroot environment. The symlinks can
  be broken if the server modifies the target path. But Rclone can't be able
  to fix this automatically.

- **Modification times**: Symlink modification times are not preserved, as
  most SFTP servers don't support the `lutimes` operation for symlinks.

- **Checksums**: Checksums are calculated based on the symlink target path, not the content.
  Normalization by the server may affect verification.

- **SFTPGo behavior**: When testing with SFTPGo, we observed that it can modify
  symlink target paths in ways that may break links when transferring between servers:
  - Absolute symlinks: A symlink on an OpenSSH server pointing to /home/foo/file.txt
    may become /home/foo/home/foo/file.txt on SFTPGo because of its chroot environment,
    resulting in a broken link.
  - Relative symlinks: Relative symlinks usually work when copied from OpenSSH to SFTPGo.
    However, copying them back from SFTPGo to another server (OpenSSH or SFTPGo) can break
    the link because SFTPGo returns an absolute path based on its chroot
    rather than preserving the original relative path.
> Note: This behavior was observed on SFTPGo, but other SFTP servers
> could potentially exhibit similar issues depending on their implementation of symlink
> handling. Absolute paths are especially prone to breakage,
> and relative paths may not always round-trip cleanly between servers.
