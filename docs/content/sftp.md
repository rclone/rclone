...
backend, this means that on the very first access rclone will try
to run the command.

### Working with Windows SFTP servers

When working with Windows SFTP servers (like Bitvise SSH Server), you may encounter issues with characters that are forbidden in Windows file names, such as `:`, `*`, `?`, and `"`. 

To handle these characters properly, you can use the `encoding` option:
...
user = username
pass = password
encoding = Slash,BackSlash,Ctl,Dot,RightSpace,RightPeriod,InvalidUtf8,Colon,Question,DoubleQuote,Asterisk,LtGt,Pipe
```

Or use the `--sftp-encoding` flag on the command line:

```
rclone copy /path/to/local windows_sftp: --sftp-encoding "Slash,BackSlash,Ctl,Dot,RightSpace,RightPeriod,InvalidUtf8,Colon,Question,DoubleQuote,Asterisk,LtGt,Pipe"
```

This will properly encode characters that Windows doesn't support in filenames.
