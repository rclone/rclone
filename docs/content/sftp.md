...
The SFTP backend supports encoding to handle filenames with special characters
that are not compatible with the remote filesystem. This is particularly useful
when syncing to Windows SFTP servers, which don't support certain characters
in filenames such as `:`, `?`, `"`, `*`, `<`, `>`, and `|`, and also silently
remove trailing spaces and periods from filenames.

By default, encoding is disabled (`encoding = None`), which maintains backward
compatibility. When you enable encoding, rclone will automatically translate
incompatible characters to their fullwidth Unicode equivalents when sending
files to the server, and translate them back when listing files.

### Example: Syncing to Windows SFTP Server

If you're syncing files to a Windows server via SFTP and have files with
colons or other Windows-incompatible characters in their names, you can
configure encoding like this:
...
```console
rclone config create windows-sftp sftp \
  host=windows-server.example.com \
  user=myuser \
  pass=mypass \
  encoding=Win
...
With this configuration, a file named `test:file.txt` will be automatically
encoded as `test：file.txt` (with a fullwidth colon) on the Windows server,
but will appear as `test:file.txt` when you list it with rclone.

The `Win` encoding handles all Windows-incompatible characters including:
`:`, `?`, `"`, `*`, `<`, `>`, `|`, trailing spaces, and trailing periods.

See the [encoding section in the overview](/overview/#encoding) for more
information about available encoding options.
