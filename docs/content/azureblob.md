...
If `use_device_code_flow` is set a device code is generated and the default browser is launched. The device code must be entered in the browser before entering credentials.

`env_auth` needs to be unset to use this.

#### Authorization code flow

If `use_authorization_flow` is set then the user is prompted to enter the credentials in the default browser. 
