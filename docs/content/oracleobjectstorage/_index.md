...
### Listing resume and checkpoints

The oracleobjectstorage backend supports two advanced options for long-running
list operations:

- `--oos-list-start`
- `--oos-list-checkpoint-file`

These serve different purposes.

`--oos-list-start` is a manual override which sets the initial OCI
`ListObjects.Start` value. It is useful when an external caller already knows
the exact OCI pagination marker to resume from and wants a stateless resume
mechanism.

If the marker is invalid or the referenced object no longer exists, rclone
fails the listing with a structured error message. The caller can then decide
whether to retry from the beginning.

`--oos-list-checkpoint-file` is the preferred backend-managed resume mechanism.
When set, rclone reads the checkpoint file before listing and writes updated
progress back to it after each listed page.

The checkpoint file contains more than just the pagination marker. It also
stores the listing scope:
...
This allows rclone to reject stale or mismatched checkpoints instead of
accidentally applying a marker from one listing to another.

Checkpoint loading is version-tolerant for backward compatibility. If future
schema changes still preserve the fields needed for resume, rclone will attempt
to load the checkpoint despite a stored version mismatch.

If the checkpoint file is corrupted, invalid for the current listing scope, or
the stored marker points to an object that has since been deleted, rclone fails
the listing with a structured error message. The caller can then decide whether
to retry from the beginning.

The checkpoint file is meant to be consumed directly by rclone. External
applications should generally pass the same file back with
`--oos-list-checkpoint-file`.

If both options are supplied, then a matching checkpoint file takes precedence
over `--oos-list-start`. This is also true when the checkpoint marker is empty,
which means "resume from the beginning".
...
Start listing from this object name marker.
...
- Config:      list_start
- Env Var:     RCLONE_OOS_LIST_START
- Type:        string
- Required:    false

#### --oos-list-checkpoint-file

Path to a local checkpoint file used during listing.
...
- Config:      list_checkpoint_file
- Env Var:     RCLONE_OOS_LIST_CHECKPOINT_FILE
- Type:        string
- Required:    false
