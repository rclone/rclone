...
`rclone config` walks you through the trust token creation. The trust token is valid
for 30 days. After which you will have to reauthenticate. See
...
You should then run `rclone config reconnect remote:`.
...
clearing the config entry and running `rclone config reconnect remote:` until rclone
functions properly.

### Reauthentication {#reauthentication}

```text
CRITICAL: Failed to create file system for "icloud:/": HTTP error 400
(400 Bad Request) returned body: "{\"success\":false,\"error\":\"Invalid
Session Token\"}"
```

Your session with iCloud has expired and needs renewing. You will need to 
reauthenticate rclone with your iCloud Drive.

```bash 
rclone config reconnect [your-icloud-remote]:
# for example:
# rclone config reconnect iclouddrive:
```

#### Known issue bug #8587

[rclone bug #8587](https://github.com/rclone/rclone/issues/8587]: iCloudDrive 
currently rejects rclone's default user agent string. To workaround this, 
provide your own user string with the `--user-agent string` flag. A random 
string of characters works fine at time of writing.

```bash 
rclone config reconnect [your-icloud-remote]: --user-agent [random-string]
# for example:
# rclone config reconnect iclouddrive: --user-agent aosetuh
```
