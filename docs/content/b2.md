...
`--b2-upload-concurrency` (8 by default) buffers of `--b2-chunk-size`
(16 MiB by default) each.  There can be at most `--transfers` files
uploading at any moment, so this sets the upper limit on the memory
used.
