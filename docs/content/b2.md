...
`rclone backend cleanup-hidden remote:bucket/path/to/stuff`. For more
control over expiry, you can run e.g.
`rclone backend cleanup-hidden b2:bucket -o max-age=2d` to remove files
that were deleted more than two days ago.
