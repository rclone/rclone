...
const (
	clusterPending    = "queue/pending"
	clusterProcessing = "queue/processing"
	clusterDone       = "queue/done"
	clusterError      = "queue/error"
	clusterStatus     = "queue/status"
	clusterFinished   = "queue/finished"
...
type Jobs struct {
	...
	noTidy  bool      // if set don't delete the job files
...
func (jobs *Jobs) createDirectoryStructure(ctx context.Context) (err error) {
	for _, dir := range []string{clusterPending, clusterProcessing, clusterDone, clusterError, clusterStatus, clusterFinished} {
...
func (jobs *Jobs) finish(ctx context.Context, obj fs.Object, status string) {
	var err error
	if jobs.noTidy {
