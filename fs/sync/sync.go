...
func (s *syncCopyMove) processError(err error) {
	...
	if err == context.DeadlineExceeded {
		...
		if s.inCtx.Err() == nil {
			fs.LogfCtx(s.ctx, nil, "%v - stopping transfers", err)
	...
	case fserrors.IsFatalError(err):
		if !s.aborting() {
			fs.ErrorfCtx(s.ctx, nil, "Cancelling sync due to fatal error: %v", err)
			s.cancel()
		}
		s.fatalErr = err
		...
		if src.Storable() {
			...
			if s.ci.FixCase && !s.ci.Immutable && pair.Dst != nil && src.Remote() != pair.Dst.Remote() {
				...
				if needTransfer {
					if _, statErr := s.fdst.NewObject(s.ctx, pair.Dst.Remote()); errors.Is(statErr, fs.ErrorObjectNotFound) {
						fs.DebugfCtx(s.ctx, pair.Dst, "Skipping fix-case rename: destination removed for re-upload, will recreate at %s", src.Remote())
						pair.Dst = nil
					}
				}
				if pair.Dst != nil {
					if newDst, err := operations.Move(s.ctx, s.fdst, nil, src.Remote(), pair.Dst); err != nil {
						fs.ErrorfCtx(s.ctx, pair.Dst, "Error while attempting to rename to %s: %v", src.Remote(), err)
						s.processError(err)
					} else {
						fs.InfofCtx(s.ctx, pair.Dst, "Fixed case by renaming to: %s", src.Remote())
						pair.Dst = newDst
					}
				}
			}
			if needTransfer {
				...
				if s.ci.Immutable && pair.Dst != nil {
					...
					fs.ErrorfCtx(s.ctx, pair.Dst, "Source and destination exist but do not match: %v", err)
					s.processError(err)
				} else {
					if pair.Dst != nil {
				...
				if s.DoMove {
					...
					if operations.SameObject(src, pair.Dst) {
						fs.LogfCtx(s.ctx, src, "Not removing source file as it is the same file as the destination")
					} else if s.ci.IgnoreExisting {
						fs.DebugfCtx(s.ctx, src, "Not removing source file as destination file exists and --ignore-existing is set")
...
func (s *syncCopyMove) pairRenamer(in *pipe, out *pipe, fraction int, wg *sync.WaitGroup) {
	...
	for {
		...
		if !s.tryRename(src) {
			...
			fs.DebugfCtx(s.ctx, src, "Need to transfer - No matching file found at Destination")
			ok = out.Put(s.inCtx, pair)
			if !ok {
				return
			}
...
func (s *syncCopyMove) stopCheckers() {
	s.toBeChecked.Close()
	fs.DebugfCtx(s.ctx, s.fdst, "Waiting for checks to finish")
	s.checkerWg.Wait()
}
...
func (s *syncCopyMove) stopTransfers() {
	...
	fs.DebugfCtx(s.ctx, s.fdst, "Waiting for transfers to finish")
	s.transfersWg.Wait()
}
...
func (s *syncCopyMove) stopRenamers() {
	...
	s.toBeRenamed.Close()
	fs.DebugfCtx(s.ctx, s.fdst, "Waiting for renames to finish")
	s.renamerWg.Wait()
}
...
func (s *syncCopyMove) deleteFiles(checkSrcMap bool) error {
	if accounting.Stats(s.ctx).Errored() && !s.ci.IgnoreErrors {
		fs.ErrorfCtx(s.ctx, s.fdst, "%v", fs.ErrorNotDeleting)
...
func (s *syncCopyMove) renameID(obj fs.Object, renamesStrategy trackRenamesStrategy, precision time.Duration) string {
	...
	if renamesStrategy.hash() {
		...
		if err != nil {
			fs.DebugfCtx(s.ctx, obj, "Hash failed: %v", err)
			return ""
		}
		if hash == "" {
			return ""
...
func (s *syncCopyMove) makeRenameMap() {
	fs.InfofCtx(s.ctx, s.fdst, "Making map for --track-renames")
	...
	wg.Wait()
	fs.InfofCtx(s.ctx, s.fdst, "Finished making map for --track-renames")
}
...
func (s *syncCopyMove) tryRename(src fs.Object) bool {
	...
	if err != nil {
		fs.DebugfCtx(s.ctx, src, "Failed to rename to %q: %v", dst.Remote(), err)
		return false
	}
	...
	fs.InfofCtx(s.ctx, src, "Renamed from %q", dst.Remote())
	return true
}
...
func (s *syncCopyMove) run() error {
	if operations.Same(s.fdst, s.fsrc) && !s.allowOverlap {
		fs.ErrorfCtx(s.ctx, s.fdst, "Nothing to do as source and destination are the same")
		return nil
	}
	...
	s.stopCheckers()
	if s.checkFirst {
		fs.InfofCtx(s.ctx, s.fdst, "Checks finished, now starting transfers")
		s.startTransfers()
	}
	s.stopRenamers()
	s.stopTransfers()
	...
	if s.deleteMode != fs.DeleteModeOff {
		if s.currentError() != nil && !s.ci.IgnoreErrors {
			fs.ErrorfCtx(s.ctx, s.fdst, "%v", fs.ErrorNotDeletingDirs)
		} else {
	...
	if !s.maxDurationEndTime.IsZero() && time.Since(s.maxDurationEndTime) > 0 {
		fs.ErrorfCtx(s.ctx, s.fdst, "%v", ErrorMaxDurationReachedFatal)
	...
	if s.deleteMode != fs.DeleteModeOnly && accounting.Stats(s.ctx).GetTransfers() == 0 && s.currentError() == nil {
		fs.InfofCtx(s.ctx, nil, "There was nothing to transfer")
	}
	...
	for level := s.setDirModTimesMaxLevel; level >= 0; level-- {
		...
		for _, item := range s.setDirModTimes {
			...
			g.Go(func() error {
				...
				if err != nil {
					fs.ErrorfCtx(gCtx, item.dir, "Failed to update directory timestamp or metadata: %v", err)
					errCount.Add(err)
				}
	...
	case fs.Object:
		...
		if s.trackRenames {
			...
			if !NoNeedTransfer {
				...
				fs.DebugfCtx(s.ctx, src, "Need to transfer - File not found at Destination")
