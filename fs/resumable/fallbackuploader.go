...
func (f *fallbackUploader) putSizeObject(data io.Reader, src *uploadSrcObjectInfo) (err error) {
	if f.partialFallback {
...
func (f *fallbackUploader) flush() (finished bool, err error) {
	finished = false
	if f.partialFallback {
...
func (f *fallbackUploader) finish(fragments fs.Objects) (err error) {
	if f.partialFallback {
...
func (f *fallbackUploader) Abort() (err error) {
