...
func (f *fragmentUploader) lastDir() (lastDir fs.Directory, fs_ fs.Fs, dirIndex uint64, err error) {
...
func (f *fragmentUploader) lastFragment() (frag *fragment, err error) {
...
func (f *fragmentUploader) putSizeObject(data io.Reader, src *uploadSrcObjectInfo) (err error) {
	_, err = f.fs.Put(f.ctx, data, src)
	return
}

func (f *fragmentUploader) SetSize(size int64) (err error) {
	if size > 1 {
...
func (f *fragmentUploader) getAllFragments(fs_ fs.Fs, uploadDir string) (fragments []*fragment, err error) {
...
func (f *fragmentUploader) flush() (finished bool, err error) {
	finished = false
	pos, ok := f.self.Pos()
	if !ok {
		err = errors.New("failed to determine current upload position")
	}
	...
	if err == nil {
		size, ok := f.Size()
		finished = ok && size == pos
	}
	return
}

func (f *fragmentUploader) Close() (err error) {
	...
	if err == nil && finished {
		...
		for i := range fragments {
			if fragments[i].start+fragments[i].object.Size() != fragments[i+1].start {
				err = errors.New(fmt.Sprintf("missing upload fragment between %d and %d", fragments[i].start+fragments[i].object.Size(), fragments[i+1].start))
				return
			}
...
func (f *fragmentUploader) Abort() (err error) {
