...
func TestBufferPool(t *testing.T) {
	bp := newBufferPool(60*time.Second, 2)

	assert.Equal(t, 0, bp.buffersInUse())

	b1 := bp.get()
	assert.Equal(t, 1, bp.buffersInUse())

	b2 := bp.get()
	assert.Equal(t, 2, bp.buffersInUse())

	b3 := bp.get()
	assert.Equal(t, 3, bp.buffersInUse())
...
func TestBufferPoolFlusher(t *testing.T) {
	bp := newBufferPool(50*time.Millisecond, 2)
