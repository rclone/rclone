...
func (h *ReOpen) open() error {
	...
	if h.err != nil {
		if h.tries > 1 {
			fs.DebugfCtx(h.ctx, h.src, "Reopen failed after offset %d bytes read: %v", h.offset, h.err)
		}
		return h.err
	}
	h.opened = true
...
func (h *ReOpen) Read(p []byte) (n int, err error) {
	...
	if h.newOffset >= 0 {
		if h.offset != h.newOffset {
			fs.DebugfCtx(h.ctx, h.src, "Seek from %d to %d", h.offset, h.newOffset)
	...
	for n < len(p) && err == nil {
		...
		if err != nil && err != io.EOF {
			...
			if !fserrors.IsNoLowLevelRetryError(err) {
				fs.DebugfCtx(h.ctx, h.src, "Reopening on read failure after offset %d bytes: retry %d/%d: %v", h.offset, h.tries, h.maxTries, err)
				if h.reopen() == nil {
					err = nil
				}
			}
