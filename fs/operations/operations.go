...
func Copy(ctx context.Context, f fs.Fs, dst fs.Object, remote string, src fs.Object) (newDst fs.Object, err error) {
	...
	if !ci.Inplace && f.Features().Move != nil && f.Features().PartialUploads && !strings.HasSuffix(remote, ".rclonelink") {
		...
		ext := strings.TrimSpace(ci.PartialUploadExtension)
		if len(ext) == 0 {
			ext = fs.GetConfig(context.Background()).PartialUploadExtension
		}

		suffix := "." + random.String(8) + "." + ext
		base := path.Base(remotePartial)
		if len(base) > 100 {
			limit := len(base) - len(suffix)

			if len(suffix) > len(base) {
				limit = 1
			}

			remotePartial = remotePartial[:len(remotePartial)-len(base)+limit] + suffix
		} else {
