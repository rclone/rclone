...
func MkdirModTime(ctx context.Context, f fs.Fs, dir string, modTime time.Time) (newDst fs.Directory, err error) {
	...
	fs.InfofCtx(ctx, logName, "Made directory with modification time %v", modTime)
	return newDst, err
}
...
func Cat(ctx context.Context, f fs.Fs, w io.Writer, offset, count int64, sep []byte) error {
	...
	return ListFn(ctx, f, func(o fs.Object) {
		...
		if err != nil {
			err = fs.CountError(ctx, err)
			fs.ErrorfCtx(ctx, o, "Failed to open: %v", err)
			return
		}
		if count >= 0 {
	...
	for level := len(toDelete) - 1; level >= 0; level-- {
		...
		for _, dir := range dirs {
			...
			g.Go(func() error {
				...
				if err != nil {
					err = fs.CountError(ctx, err)
					fs.ErrorfCtx(ctx, dir, "Failed to rmdir: %v", err)
					errCount.Add(err)
				}
