...
func useFsLog(m dsl.Matcher) {
	m.Match(`log.Print($x)`, `log.Println($x)`).Where(m["x"].Type.Is(`string`)).Suggest(`fs.Printf(nil, $x)`)
	m.Match(`log.Print($*args)`, `log.Println($*args)`).Suggest(`fs.Printf(nil, fmt.Sprint($args))`)
	m.Match(`log.Printf($*args)`).Suggest(`fs.Printf(nil, $args)`)
