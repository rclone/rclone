...
// NewTransformationsClient creates an instance of the TransformationsClient
// client.
...
// NewTransformationsClientWithBaseURI creates an instance of the
// TransformationsClient client.
...
// CreateOrReplace creates a transformation or replaces an already existing
// transformation under an existing streaming job.
//
// transformation is the definition of the transformation that will be used to
// create a new transformation or replace the existing one under the streaming
// job. resourceGroupName is the name of the resource group that contains the
// resource. You can obtain this value from the Azure Resource Manager API or
// the portal. jobName is the name of the streaming job. transformationName is
// the name of the transformation. ifMatch is the ETag of the transformation.
// Omit this value to always overwrite the current transformation. Specify the
// last-seen ETag value to prevent accidentally overwritting concurrent
// changes. ifNoneMatch is set to '*' to allow a new transformation to be
...
// Update updates an existing transformation under an existing streaming job.
// This can be used to partially update (ie. update one or two properties) a
// transformation without affecting the rest the job or transformation
// definition.
//
// transformation is a Transformation object. The properties specified here
// will overwrite the corresponding properties in the existing transformation
// (ie. Those properties will be updated). Any properties that are set to null
// here will mean that the corresponding property in the existing
// transformation will remain the same and not change as a result of this PATCH
// operation. resourceGroupName is the name of the resource group that contains
// the resource. You can obtain this value from the Azure Resource Manager API
// or the portal. jobName is the name of the streaming job. transformationName
// is the name of the transformation. ifMatch is the ETag of the
// transformation. Omit this value to always overwrite the current
// transformation. Specify the last-seen ETag value to prevent accidentally
