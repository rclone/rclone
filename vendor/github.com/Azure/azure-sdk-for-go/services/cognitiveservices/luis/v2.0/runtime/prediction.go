...
// is 500 characters.
//
// appID is the LUIS application ID (Guid). query is the utterance to predict. timezoneOffset is the timezone offset
// for the location of the request. verbose is if true, return all intents instead of just the top scoring intent.
// staging is use the staging endpoint slot. spellCheck is enable spell checking. bingSpellCheckSubscriptionKey is the
// subscription key to use when enabling bing spell check logParameter is log query (default is true)
