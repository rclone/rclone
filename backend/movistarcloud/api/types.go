...
func (e *Error) Error() string {
	out := fmt.Sprintf("Error %q", e.Code)
	if e.Status != 0 {
...
//
// The API returns HTTP 200 even for some failures, signalling the problem via
// the Error field in place of the expected payload, so callers should check
// Error before decoding data.
...
// StorageInfo describes the account's storage usage as returned by the
// get-storage-space endpoint. All byte counts are in bytes; SoftDeleted is a
// count of items in the trash, not a number of bytes. NoLimit is true when the
// account has no storage quota, in which case Quota and Free are not meaningful.
