...
#
# Go tests (TestRebuild* in raid3_rebuild_test.go) cover local temp-dir and MinIO
# (TestRebuildMinioBackendSuccess with -remote TestRaid3Minio:); this script adds
# black-box validation for local and SFTP only (compare_all.sh).
#
...
# as expected) for local and SFTP configurations.
#
# Usage:
#   compare_rebuild.sh [options] <command> [args]
#
# Commands:
...
Available rebuild scenarios:
  Local:  Covered by Go tests (backend/raid3/raid3_rebuild_test.go). Bash skips run for --storage-type=local.
  MinIO/mixed: Covered by Go test TestRebuildMinioBackendSuccess (-remote TestRaid3Minio:). Bash skips for --storage-type=minio and --storage-type=mixed.
  SFTP:        Bash runs even/odd/parity (success + failure) with --storage-type=sftp.
...
main() {
  ...
  case "${COMMAND}" in
    ...
    test)
      set_remotes_for_storage_type
      if [[ "${STORAGE_TYPE}" == "local" ]]; then
        log_info "main" "Local rebuild scenarios are covered by Go tests (backend/raid3/raid3_rebuild_test.go). Skipping bash rebuild scenarios for --storage-type=local."
        reset_scenario_results
        pass_scenario "local" "Covered by Go tests (skipped)."
        print_scenario_summary
        exit 0
      fi
      if [[ "${STORAGE_TYPE}" == "minio" || "${STORAGE_TYPE}" == "mixed" ]]; then
        log_info "main" "MinIO/mixed rebuild scenarios are covered by Go test TestRebuildMinioBackendSuccess (-remote TestRaid3Minio:). Skipping bash rebuild scenarios for --storage-type=${STORAGE_TYPE}."
        reset_scenario_results
        pass_scenario "${STORAGE_TYPE}" "Covered by Go tests (skipped)."
        print_scenario_summary
        exit 0
      fi
      [[ "${STORAGE_TYPE}" != "sftp" ]] || ensure_sftp_containers_ready
