...
#
# Usage:
#   compare_heal.sh [options] <command> [args]
#
# Commands:
...
Available healing scenarios:
  Local:       Read-heal (even/odd/parity) and listing-does-not-heal (even-list/odd-list/parity-list) covered by Go. Bash skips for --storage-type=local.
  MinIO/mixed: Read-heal and listing-in-degraded-mode covered by Go. Bash skips for --storage-type=minio and --storage-type=mixed.
  SFTP:        Bash runs even/odd/parity and even-list/odd-list/parity-list with --storage-type=sftp.
...
main() {
  ...
  case "${COMMAND}" in
    ...
    test)
      set_remotes_for_storage_type
      if [[ "${STORAGE_TYPE}" == "local" ]]; then
        log_info "main" "Local heal scenarios (degraded read + restore and listing-does-not-heal) are covered by Go tests (backend/raid3/raid3_heal_command_test.go). Skipping bash heal scenarios for --storage-type=local."
        reset_heal_results
        record_heal_result "PASS" "local" "Covered by Go tests (skipped)."
        print_heal_summary
        exit 0
      fi
      if [[ "${STORAGE_TYPE}" == "minio" || "${STORAGE_TYPE}" == "mixed" ]]; then
        log_info "main" "MinIO/mixed heal scenarios (read-heal and listing in degraded mode) are covered by Go tests (raid3_heal_command_test.go). Skipping bash heal scenarios for --storage-type=${STORAGE_TYPE}."
        reset_heal_results
        record_heal_result "PASS" "${STORAGE_TYPE}" "Covered by Go tests (skipped)."
        print_heal_summary
        exit 0
      fi
      [[ "${STORAGE_TYPE}" != "sftp" ]] || ensure_sftp_containers_ready
