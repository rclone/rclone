...
#
# Usage:
#   compare_errors.sh [options] <command> [args]
#
# Commands:
...
Available error scenarios:
  Local:  Covered by Go tests (backend/raid3/raid3_errors_test.go). Bash skips for --storage-type=local.
  MinIO/mixed/SFTP: Bash runs all scenarios (upload-fail, move-fail, update-fail, rollback-disabled-*).
  ...
  if [[ "${dest_existed}" -eq 1 ]]; then
    ...
    if [[ "${dest_still_exists}" -eq 1 ]]; then
      ...
      if [[ "${final_dest_exists}" -eq 1 ]]; then
        record_error_result "FAIL" "move-fail-${backend}" "Leftover destination could not be removed. Run: ./compare_errors.sh teardown --storage-type=${STORAGE_TYPE} && ./compare_errors.sh test --storage-type=${STORAGE_TYPE}"
        return 1
      fi
    fi
  fi
...
main() {
  ...
  case "${COMMAND}" in
    ...
    test)
      set_remotes_for_storage_type
      if [[ "${STORAGE_TYPE}" == "local" ]]; then
        log_info "main" "Local error scenarios (upload-fail, move-fail, update-fail, rollback-disabled) are covered by Go tests (backend/raid3/raid3_errors_test.go). Skipping bash error scenarios for --storage-type=local."
        reset_error_results
        record_error_result "PASS" "local" "Covered by Go tests (skipped)."
        print_error_summary
        exit 0
      fi
      [[ "${STORAGE_TYPE}" != "minio" && "${STORAGE_TYPE}" != "mixed" ]] || ensure_minio_containers_ready
