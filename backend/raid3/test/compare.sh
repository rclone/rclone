...
# (optionally) shows both outputs.
#
# For environment maintenance (start/stop containers, teardown, recreate), use manage.sh.
#
# Usage:
#   compare.sh [options] <command> [args]
#
# Commands:
...
#
# Options:
#   --storage-type <local|minio|mixed|sftp>   Select which backend pair (required for test).
...
Options:
  --storage-type <local|minio|mixed|sftp>   Select backend pair (required for test).
  -v, --verbose                  Show stdout/stderr from both rclone invocations.
  -h, --help                     Display this help.

For start/stop containers, teardown, or recreate, use: ./manage.sh <command> --storage-type=...
...
parse_args() {
  ...
  case "${COMMAND}" in
    test)
      [[ -n "${STORAGE_TYPE}" ]] || die "--storage-type must be provided for 'test'"
      ;;
  esac
...
Available tests:
  ...
  performance  Compare upload/download performance between raid3 and single remotes.
               PASS: raid3 is not more than 20x slower than single (ratio <= 20x).
               FAIL: raid3 is more than 20x slower than single (ratio > 20x).
EOF
}
...
run_performance_test() {
  ...
  local upload_ratio max_ratio="20"
  ...
  if (( $(LC_NUMERIC=C awk "BEGIN {print (${single_time} < 0.001)}") )); then
    ...
    # Check if raid3 upload is more than 20x slower (ratio > 20)
  ...
  if (( $(LC_NUMERIC=C awk "BEGIN {print (${single_dl_time} < 0.001)}") )); then
    ...
    # Check if raid3 download is more than 20x slower (ratio > 20)
...
run_mkdir_test() {
  ...
  purge_raid3_remote_root
  purge_remote_root "${SINGLE_REMOTE}"
  # After purge, MinIO (especially odd for mixed) can be slow; give it time and re-verify so mkdir does not see DEGRADED.
  if [[ "${STORAGE_TYPE}" == "minio" || "${STORAGE_TYPE}" == "mixed" ]]; then
    sleep 3
    if [[ "${STORAGE_TYPE}" == "mixed" ]]; then
      # Mixed uses MinIO only for odd; re-verify it is responsive after purge before mkdir.
      if ! wait_for_minio_backend_ready "odd"; then
        log_warn "test:${test_case}" "MinIO odd backend did not become ready after purge; mkdir may fail with DEGRADED."
      fi
    else
      # Full minio: re-verify all three raid3 backends so no stale DEGRADED state after purge.
      for b in even odd parity; do
        wait_for_minio_backend_ready "${b}" || true
      done
    fi
  fi
  local test_id
