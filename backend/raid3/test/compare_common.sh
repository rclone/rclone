...
# This file is sourced by compare*.sh variants (compare.sh, compare_heal.sh, etc.).
...
if [[ ! -f "${SCRIPT_DIR}/compare_raid3_env.sh" ]]; then
  printf '[%s] ERROR: Missing required env file: %s\n' "${SCRIPT_NAME:-compare_common.sh}" "${SCRIPT_DIR}/compare_raid3_env.sh" >&2
  exit 1
fi
...
ensure_rclone_config() {
  # Verify that tests.config exists
...
# When STORAGE_TYPE is minio or mixed, adds HTTP timeouts so S3 ops don't hang indefinitely.
# --timeout must be long enough for CreateBucket/mkdir on slow MinIO (e.g. Docker on Mac);
# 30s was too short and caused "context deadline exceeded" for mixed. Use 90s (or RCLONE_HTTP_TIMEOUT).
...
rclone_cmd_raw() {
  ...
  if [[ "${STORAGE_TYPE:-}" == "minio" || "${STORAGE_TYPE:-}" == "mixed" ]]; then
    local contimeout="${RCLONE_CONTIMEOUT:-15s}"
    local timeout="${RCLONE_HTTP_TIMEOUT:-90s}"
    "${RCLONE_BINARY}" --config "${RCLONE_CONFIG}" --retries 1 --contimeout "${contimeout}" --timeout "${timeout}" "${quiet[@]+"${quiet[@]}"}" "$@"
  else
...
# Check that Docker is available and the daemon is accessible (for minio/sftp/mixed).
# Call before ensure_minio_containers_ready or ensure_sftp_containers_ready.
check_docker_available() {
  if ! command -v docker >/dev/null 2>&1; then
    die "Docker is required for --storage-type=minio, --storage-type=mixed, and --storage-type=sftp." \
        "Docker was not found in PATH. Please install Docker and ensure it is running."
  fi
  if ! docker info >/dev/null 2>&1; then
    local err
    err=$(docker info 2>&1 | tail -1) || err="unknown"
    die "Docker is required for --storage-type=minio, --storage-type=mixed, and --storage-type=sftp." \
        "Cannot connect to the Docker daemon. Ensure Docker is running and you have permission to access it (e.g. add your user to the 'docker' group)." \
        "Error: ${err}"
  fi
}
...
wait_for_minio_backend_ready() {
  ...
  case "${backend}" in
    ...
    single) remote="${MINIO_SINGLE_REMOTE}" ;;
    *) return 1 ;;
  esac

  local retries=30
  local delay=1
  # Use a short timeout per attempt so we retry quickly instead of waiting RCLONE_TEST_TIMEOUT (e.g. 120s) each time
  ...
  while (( retries > 0 )); do
    ...
    # Connection/timeout errors mean backend is not ready yet - keep retrying
    if echo "${output}" | grep -qiE "(connection reset|connection refused|no route to host|timeout|deadline exceeded)"; then
      sleep "${delay}"
      (( retries-- ))
      continue
    fi
...
ensure_minio_containers_ready() {
  ...
  # Wait for MinIO S3 API to be ready (not just TCP). Avoids "context deadline exceeded" on first mkdir/ls.
  log_info "autostart" "Waiting for MinIO S3 API to be ready..."
  for backend in even odd parity; do
    if ! wait_for_minio_backend_ready "${backend}"; then
      log_fail "autostart" "MinIO backend '${backend}' did not become ready in time. Run: docker logs $(minio_container_for_backend ${backend})"
      return 1
    fi
  done
  if [[ "${STORAGE_TYPE}" == "minio" ]]; then
    if ! wait_for_minio_backend_ready "single"; then
      log_fail "autostart" "MinIO backend 'single' did not become ready in time. Run: docker logs ${MINIO_SINGLE_NAME}"
      return 1
    fi
  fi
...
# remove_minio_containers stops and removes all MinIO containers so they can be
# recreated from scratch (e.g. to fix broken container state). Use with recreate command.
...
# remove_sftp_containers stops and removes all SFTP containers so they can be
# recreated from scratch. Use with recreate command.
...
# For mixed: same for even/parity (local), empty for odd (MinIO bucket root).
# For minio/sftp: empty so we use the remote root.
backend_raid3_root_path() {
  ...
  case "${STORAGE_TYPE}" in
    local)
      ...
      ;;
    mixed)
      # Even and parity are local: scope to their data dirs so we only purge test data, not the whole fs.
      case "${backend}" in
        even|parity)
          local dir
          dir=$(remote_data_dir "${backend}")
