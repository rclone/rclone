...
**References**: `docs/TEST_DOCUMENTATION_PROPOSAL.md`, `TESTING.md`
...
- `go test ./backend/raid3/...` runs the full integration suite with temporary local remotes.
- For MinIO-based testing, run the three containers defined in `rclone.conf`, then execute `go test ./backend/raid3`.
