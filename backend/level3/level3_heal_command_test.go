...
// createTestFile uploads a single file to the level3 backend and returns its remote path.
func createTestFile(t *testing.T, ctx context.Context, f *level3.Fs, name string, data []byte) string {
	t.Helper()
...
func TestHealCommandReconstructsMissingParticle(t *testing.T) {
	...
	// Create level3 Fs over the three local backends
	fsInterface, err := level3.NewFs(ctx, "healTest", "", configmap.Simple{
		"even":   evenDir,
		"odd":    oddDir,
		"parity": parityDir,
