...
// RoundTrip adds the Bearer token to every http request
...
func (f *Fs) PutUnchecked(ctx context.Context, in io.Reader, src fs.ObjectInfo, options ...fs.OpenOption) (fs.Object, error) {
	...
	leaf, directoryID, err := f.dirCache.FindPath(ctx, src.Remote(), false)
	if err != nil {
		return nil, err
	}

	_, err = f.uploadFile(in, src.Size(), leaf, directoryID, nodeResponse.ID, nodeResponse.URL)
	if err != nil {
		return nil, err
	}
...
func (f *Fs) Rmdir(ctx context.Context, dir string) error {
	...
	_, did, err := f.dirCache.FindPath(ctx, f.root, false)
	if err != nil {
		return err
	}
