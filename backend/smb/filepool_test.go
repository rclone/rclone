...
func (m *mockFs) isClosed(c *conn) bool {
	if m.closedFn != nil {
		return m.closedFn(c)
	}
	return false
...
// Test draining pool with dead connections
func TestFilePool_Drain_WithDeadFiles(t *testing.T) {
	ctx := context.Background()
	mfs := newMockFs()
	mfs.closedFn = func(c *conn) bool { return true }
	pool := newFilePool(ctx, mfs, "testshare", "/test/path")
