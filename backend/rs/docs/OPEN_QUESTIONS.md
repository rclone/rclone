...
- **WriteID guard (plan a)**: Each `Put` stamps a shared random 64-bit **`WriteID`** on every shard footer. Read and heal use the same **`probeAndSelectWriteIDGroup`** rule: pick the unique WriteID group with **≥ k** layout-compatible shards; reconstruct via parity when needed; error if none reach k. Requires **k > m** for uniqueness. Footer v1 redefined in place (104 bytes); no dual-parse of old 96-byte footers.
