...
func UploadMultipart(ctx context.Context, src fs.ObjectInfo, in io.Reader, opt UploadMultipartOptions) (chunkWriterOut fs.ChunkWriter, err error) {
	...
	// backend. Returning an error here aborts the upload. The check is
	// skipped with --ignore-size for sources which misreport their size.
	if size >= 0 && off != size && !fs.GetConfig(ctx).IgnoreSize {
