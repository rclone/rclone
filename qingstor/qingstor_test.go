...
func TestSetup(t *testing.T) {
	...
	fstests.RemoteName = "TestQS:"
}

// Generic tests for the Fs
func TestInit(t *testing.T)                { fstests.TestInit(t) }
func TestFsString(t *testing.T)            { fstests.TestFsString(t) }
// Temporary skip the following two test case, Because allowed only perform the creation and \
// deletion for the same bucket after a period of time.
//func TestFsRmdirEmpty(t *testing.T)        {time.Sleep(time.Second * 3) fstests.TestFsRmdirEmpty(t) }
//func TestFsRmdirNotFound(t *testing.T)     {time.Sleep(time.Second * 120) fstests.TestFsRmdirNotFound(t) }
