...
func New(item Item, opt *vfscommon.Options, remote string, src fs.Object) (dls *Downloaders) {
	...
	dls.wg.Go(func() {
		...
		case <-ticker.C:
			err := dls.kickWaiters()
			if err != nil {
				fs.ErrorfCtx(ctx, dls.src, "vfs cache: failed to kick waiters: %v", err)
			}
		case <-ctx.Done():
			break
		}
...
func (dls *Downloaders) _countErrors(n int64, err error) {
	if err == nil && n != 0 {
		if dls.errorCount != 0 {
			fs.InfofCtx(dls.ctx, dls.src, "vfs cache: downloader: resetting error count to 0")
			dls.errorCount = 0
			dls.lastErr = nil
		}
		return
	...
	if err != nil {
		...
		//}
		dls.lastErr = err
		fs.InfofCtx(dls.ctx, dls.src, "vfs cache: downloader: error count now %d: %v", dls.errorCount, err)
	}
}
...
func (dls *Downloaders) _newDownloader(r ranges.Range) (dl *downloader, err error) {
	...
	dl.wg.Go(func() {
		...
		if err != nil {
			fs.ErrorfCtx(dls.ctx, dl.dls.src, "vfs cache: failed to download: %v", err)
		}
		err = dl.dls.kickWaiters()
		if err != nil {
			fs.ErrorfCtx(dls.ctx, dl.dls.src, "vfs cache: failed to kick waiters: %v", err)
		}
	})
...
func (dls *Downloaders) kickWaiters() (err error) {
	...
	for _, waiter := range dls.waiters {
		...
		if err != nil {
			...
			fs.ErrorfCtx(dls.ctx, dls.src, "vfs cache: restart download failed: %v", err)
		}
	}
	if fserrors.IsErrNoSpace(dls.lastErr) {
		fs.ErrorfCtx(dls.ctx, dls.src, "vfs cache: cache is out of space %d/%d: last error: %v", dls.errorCount, maxErrorCount, dls.lastErr)
		dls._closeWaiters(dls.lastErr)
		return dls.lastErr
	}

	if dls.errorCount > maxErrorCount {
		fs.ErrorfCtx(dls.ctx, dls.src, "vfs cache: too many errors %d/%d: last error: %v", dls.errorCount, maxErrorCount, dls.lastErr)
		dls._closeWaiters(dls.lastErr)
		return dls.lastErr
	}
...
func (dl *downloader) Write(p []byte) (n int, err error) {
	...
	defer func() {
		...
		if waitErr := dl.dls.kickWaiters(); waitErr != nil {
			fs.ErrorfCtx(dl.dls.ctx, dl.dls.src, "vfs cache: download write: failed to kick waiters: %v", waitErr)
			if err == nil {
				err = waitErr
			}
		}
...
loop:
	for dl.offset >= dl.maxOffset {
		...
		case <-timeout.C:
			...
			dl.mu.Lock()
			if !dl.stop {
				fs.DebugfCtx(dl.dls.ctx, dl.dls.src, "vfs cache: stopping download thread as it timed out")
				dl._stop()
			}
			break loop
		}
	...
	if !dl.stop && dl.skipped > maxSkipBytes {
		fs.DebugfCtx(dl.dls.ctx, dl.dls.src, "vfs cache: stopping download thread as it has skipped %d bytes", dl.skipped)
		dl._stop()
	}
